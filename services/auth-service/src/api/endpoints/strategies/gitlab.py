...
async def gt_auth():
    ...
    await asyncio.gather(
        add_key_value_redis(state_data.get("key"), state_data.get("state"), 600),
        add_key_value_redis(state_data.get("state"), code_verifier, 600)
    )
...
async def gt_auth_callback(request: Request, response: Response):
    ...
    state_record, code_verifier = await asyncio.gather(
        get_value_redis(state_data.get("key")),
        get_value_redis(state_data.get("state"))
    )
